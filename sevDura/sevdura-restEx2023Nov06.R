stop()
### Following is code downloaded (as Python) from computational notebook 2023Nov06:
# -*- coding: utf-8 -*-
"""sevDura.ipynb
...
c_gsFilename_which <- c(
  " - 2023Nov06-2100", # includes `medication` & `triggerTravel`
...
dat$sevDura <- create_sevDura(dat)
...
presc.summ <- lapply(levels(dat$prescription), FUN=function(presc) summary(coredata(dat$sevDura[dat$prescription == presc])))
names(presc.summ) <- levels(dat$prescription)
presc.summ.df <- cbind(round(do.call(rbind, presc.summ), 2), count=summary(dat$prescription))
colnames(presc.summ.df) <- c("Min", "Q1", "Median", "Mean", "Q3",	"Max", "Count")
presc.summ.df
